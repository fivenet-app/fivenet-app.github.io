...
Prüfen Sie vor Beginn die [Anforderungen](../2.requirements.md) und stellen Sie sicher, dass Datenbank, NATS-Server, Storage, Domain und Cluster-Ingress bereit sind.
...
Bearbeiten Sie die `values.yaml`-Datei entsprechend Ihren Bereitstellungsanforderungen. Folgen Sie der Anleitung [FiveNet konfigurieren](/de/getting-started/configuration/configure) für die erforderlichen Einstellungen in `.fivenet.config` und verwenden Sie anschließend die [Konfigurationsdatei-Referenz](/de/getting-started/reference/config-reference) für einzelne Optionen.
...
Für häufige Probleme während der Kubernetes- oder Helm-Installation, wie z. B. datenbankbezogene Fehler, lesen Sie die Seite [Datenbank- und Installationsfehlerbehebung](/de/getting-started/troubleshooting/installation-issues).
