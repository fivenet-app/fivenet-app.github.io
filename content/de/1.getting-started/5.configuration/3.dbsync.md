---
title: 'DBSync: Installieren und konfigurieren'
---

Verwenden Sie diese Anleitung, wenn FiveNet Charaktere, Fahrzeuge und andere Daten aus Ihrer Gameserver-Datenbank übernehmen soll, ohne dass der FiveNet-Server direkten Datenbankzugriff benötigt.

Schließen Sie zuerst [FiveNet konfigurieren](/de/getting-started/configuration/configure) ab, damit Datenbank, NATS-Verbindung, öffentliche URL und Sync-API-Token eingerichtet sind.
...
Falls das noch nicht der Fall ist, folgen Sie bitte zuerst der [Installationsanleitung](/de/getting-started/installation/methods).

Wenn Sie Docker für Ihre FiveNet-Bereitstellung verwenden, können Sie DBSync auch in einem Container ausführen. Siehe dazu den Beispiel-DBSync-Dienst in `docker-compose.yaml` und fahren Sie mit dem Konfigurationsabschnitt unten fort.
::
...
1. Laden Sie die neueste Version der `fivenet`-Binary oder -Exe aus [dem offiziellen FiveNet-Repository](https://github.com/fivenet-app/fivenet/releases) herunter und entpacken Sie sie.
   Verwenden Sie die Befehle für Ihre Plattform oder laden Sie das passende Archiv manuell von der Release-Seite herunter:
   ...
   # Laden Sie das Windows-amd64-Archiv von der Release-Seite herunter und entpacken Sie es.
   # Das entpackte Verzeichnis sollte fivenet.exe enthalten.
   Expand-Archive .\fivenet-windows-amd64.zip -DestinationPath .\fivenet
   ```
...
2. Erstellen Sie ein eigenes Verzeichnis für die DBSync-Binary bzw. -Exe und die Konfigurationsdateien.
   ...
   ```powershell [Windows]
   New-Item -ItemType Directory -Force 'C:\Program Files\FiveNet\DBSync'
   Move-Item .\fivenet\fivenet.exe, .\fivenet\config.example.yaml, .\fivenet\dbsync.example.yaml 'C:\Program Files\FiveNet\DBSync'
   ```

   ::

3. Optional: Fügen Sie die `fivenet`-Binary Ihrem System-`PATH` hinzu, damit Sie sie von überall ausführen können.
   ::code-group
   ...
   ```powershell [Windows]
   $dbsyncPath = 'C:\Program Files\FiveNet\DBSync'
   [Environment]::SetEnvironmentVariable('Path', $env:Path + ";$dbsyncPath", 'User')
   # Starten Sie PowerShell nach der PATH-Aktualisierung neu.
   ```
...
4. Benennen Sie die Datei `dbsync.example.yaml` in `dbsync.yaml` um.
   ...
   ```powershell [Windows]
   Set-Location 'C:\Program Files\FiveNet\DBSync'
   Rename-Item dbsync.example.yaml dbsync.yaml
   ```

   ::

5. Bearbeiten Sie die Datei `dbsync.yaml`, um DBSync zu konfigurieren. Sie können dazu jeden Texteditor verwenden, zum Beispiel:
   ::code-group
   ...
   ```powershell [Windows]
   notepad.exe dbsync.yaml
   ```

   ::

   - Folgen Sie dem [Konfigurationsabschnitt](#konfiguration) weiter unten.
6. Stellen Sie sicher, dass die Sync-API in Ihrer zentralen FiveNet-`config.yaml` aktiviert ist und mindestens ein Sync-Token konfiguriert wurde. Siehe [Konfigurationsdatei-Referenz](/de/getting-started/reference/config-reference#erforderliche-konfigurationsoptionen).
7. Sie können DBSync nun mit dem Befehl `fivenet dbsync` ausführen.
...
Öffnen Sie ein Terminal oder eine Eingabeaufforderung und navigieren Sie in das Verzeichnis, das die `fivenet`-Binary enthält:

::code-group

```bash [Linux]
cd /pfad/zu/fivenet/verzeichnis
```

```powershell [Windows]
Set-Location 'C:\pfad\zu\fivenet\verzeichnis'
```

::

Folgen Sie anschließend den plattformspezifischen Anweisungen unten, um den DBSync-Dienst zu installieren, zu starten, zu stoppen oder zu deinstallieren.

#### Dienst installieren

Um DBSync als Dienst zu installieren, befolgen Sie diese Schritte:

Unter Linux kopieren Sie die fertige Konfigurationsdatei `dbsync.yaml` vor der Installation des Dienstes nach `/etc/fivenet`. Dadurch verwendet der DBSync-Dienst beim Start die richtige Konfigurationsdatei.
...
# Oder die Windows-Diensteverwaltung direkt verwenden
