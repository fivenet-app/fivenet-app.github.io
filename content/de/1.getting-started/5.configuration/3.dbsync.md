...
1. Unter Linux: Kopieren Sie die fertige Konfigurationsdatei `dbsync.yaml` in das Verzeichnis `/etc/fivenet`.
    ```bash
    sudo mkdir -p /etc/fivenet
    sudo cp dbsync.yaml /etc/fivenet/dbsync.yaml
    ```
    Dadurch verwendet der DBSync-Dienst beim Start die richtige Konfigurationsdatei.

2. Installieren Sie den DBSync-Dienst:
    - Unter Linux:
        ```bash
