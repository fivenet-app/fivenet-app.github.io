...
Edit the `values.yaml` file to suit your deployment needs. For details about the `.fivenet.config` section, refer to the [configuration reference page](../4.configuration/1.config-reference.md#must-set-config-options), specifically the **"Must Set Config Options"** section and reference list at the top, to create a working `config.yaml` for FiveNet.
