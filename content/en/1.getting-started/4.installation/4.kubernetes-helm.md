...
Edit the `values.yaml` file to suit your deployment needs. For details about the `.fivenet.config` section, refer to the [configuration reference page](../configuration/1.config-reference.md#must-set-config-options), specifically the **"Must Set Config Options"** section and reference list at the top, to create a working `config.yaml` for FiveNet.
...
For common issues during Kubernetes or Helm installation, such as database-related errors, refer to the [Common Installation Issues](../configuration/common-installation-issues.md) page.
