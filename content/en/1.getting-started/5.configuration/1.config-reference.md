...
- `http.publicURL` - Public URL of your FiveNet instance.
- `http.sessions.cookieSecret` - Random secret used for session cookies.
- `http.sessions.domain` - Public domain or subdomain FiveNet runs on.
- `database.dsn` - MySQL or MariaDB connection string.
- `nats.url` - NATS connection URL, including credentials if needed.
- `jwt.secret` - Secret used to sign JWTs.
- `auth.configAdminGroups` - Groups that should receive config admin access in FiveNet.
- `sync.enabled` + `sync.apiTokens` - Enable the Sync API and provide at least one token. This is required for the FiveNet plugin and DBSync.
