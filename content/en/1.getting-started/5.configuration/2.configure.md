...
For every option and its default value, see the [Config File Reference](/getting-started/reference/config-reference). The example file in the FiveNet repository remains the source of truth for the complete configuration.
...
Use a database user with the permissions required for FiveNet migrations and normal operation. The configured collation should match the database and tables. See [Database Setup](/getting-started/reference/database-setup) if migrations report collation or foreign-key errors.
...
Make sure `/data` is backed by a persistent volume in Docker or Kubernetes. For multiple instances or externally managed uploads, configure the S3-compatible storage options instead. See [Advanced Configuration](/getting-started/reference/advanced).
...
You can also use `auth.configAdminUsers` for specific users. The first account still needs to be created and registered as described in [Complete Initial Setup](/getting-started/installation/post-install-steps).
...
Once the core server is working, continue with [DBSync](/getting-started/configuration/dbsync) and then [Complete Initial Setup](/getting-started/installation/post-install-steps) to connect the gameserver and configure FiveNet in the application.
