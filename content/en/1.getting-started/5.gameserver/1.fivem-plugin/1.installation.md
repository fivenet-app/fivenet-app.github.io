...
3. **Configure the Plugin**
    - Open the client `config/client.lua` and server config `config/server.lua` files included with the plugin and adjust the settings to match your server's requirements. This may include API keys, server identifiers, or other configuration options.
